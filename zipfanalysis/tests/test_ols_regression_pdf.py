...
from zipfanalysis.estimators.ols_regression_pdf import estimate_ols_regression_pdf
...
class TestOLSRegressionPDF(TestCase):
	...
	def test_with_known_exponents(self):
		...
		for alpha in [0.1, 0.5, 0.8, 1.1, 1.6, 1.9, 2.2, 2.5, 3.1]:
			...
			alpha_result = estimate_ols_regression_pdf(ns)
	...
	def test_with_low_frequency_cutoff(self):
		...
		for alpha in [0.1, 0.5, 0.8, 1.1, 1.6, 1.9, 2.2, 2.5, 3.1]:
			...
			ns = [int((r**(-1*alpha))*10000000) for r in ranks]
			ns += [1]*20
			alpha_result = estimate_ols_regression_pdf(ns, min_frequency=2)
			self.assertAlmostEqual(alpha, alpha_result, places=4)
	...
	def test_with_low_frequency_cutoff_b(self):
		...
		for alpha in [0.1, 0.5, 0.8, 1.1, 1.6, 1.9, 2.2, 2.5, 3.1]:
			...
			ns += [2]*20 + [1]*20
			print(ns)
			alpha_result = estimate_ols_regression_pdf(ns, min_frequency=3)
			self.assertAlmostEqual(alpha, alpha_result, places=4)
	...
	def test_with_floats(self):
		...
		with self.assertRaises(TypeError):
			estimate_ols_regression_pdf(ns)
